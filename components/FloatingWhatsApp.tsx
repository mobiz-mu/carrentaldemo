...
export default function FloatingWhatsApp() {
  ...
  const url = createWhatsAppChatUrl(WHATSAPP_DEFAULT_MESSAGE);
  ...
  return (
    <a
      ...
      aria-label="Chat with Mobiz Car Rental on WhatsApp"
      className="group fixed bottom-5 right-5 z-50 flex items-center gap-3 sm:bottom-6 sm:right-6"
    >
      <span className="pointer-events-none hidden rounded-full bg-ink px-3 py-2 text-xs font-semibold text-white opacity-0 shadow-lg transition-opacity duration-200 group-hover:opacity-100 sm:block">
        Chat with us
      </span>
      <span className="relative flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-xl ring-1 ring-black/5 transition-transform duration-200 hover:scale-105 hover:bg-[#1da851]">
